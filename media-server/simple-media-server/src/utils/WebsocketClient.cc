...
{
  std::string userAgent = "simple-rtmp-server";
...
void WebsocketClient::connectAsync(std::string& url, std::string& origin, std::vector<std::string>& protocols, std::string& userAgent, bool isLogger)
{
  if (mState != Disconnected) {
    // 接続中は無視します。
    LOG_WARN("WebsocketClient is already connected.\n");
    return;
  }
  mState = Connecting;
  ...
  session = soup_session_new_with_options(SOUP_SESSION_USER_AGENT, userAgent.c_str(), 
  ...
  // g_object_set(G_OBJECT(session), SOUP_SESSION_HTTPS_ALIASES, https_aliases, NULL);
...
void WebsocketClient::onServerConnected(SoupSession *session, GAsyncResult *res, gpointer userData)
{
  ...
  if (error) {
    g_error_free(error);
    if (client) {
      client->mState = Disconnected;
      if (client && client->mListener) {
        client->mListener->onFailedToConnect(client);
      }
    }
    return;
  }
...
void WebsocketClient::onServerMessage(SoupWebsocketConnection *conn, SoupWebsocketDataType type, GBytes *message, gpointer userData)
{
  ...
  switch (type) {
    case SOUP_WEBSOCKET_DATA_BINARY:
      LOG_ERROR("Received unknown binary message, ignoring.\n");
      return;
...
void WebsocketClient::loopMain()
{
  // websocket で libsoup を使用しているので、g_main_loop_run を呼び出す。
